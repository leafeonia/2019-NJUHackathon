...
public class DateTimeUtil {
    ...
    public static String getNLaterDateTimeStringUsingData(int hrs, int minute, int type) {
        Calendar c = new GregorianCalendar();
        int year = c.get(Calendar.YEAR);
        int month = c.get(Calendar.MONTH);
        int day = c.get(Calendar.DAY_OF_MONTH);
        int today = c.get(Calendar.DAY_OF_WEEK);
        Date data11 = new Date(year, month, day, hrs, minute);
        c.setTime(data11);
        c.add(Calendar.HOUR,8);
        if(today < type){
            c.add(Calendar.DAY_OF_MONTH,type-today);
        }else{
            c.add(Calendar.DAY_OF_MONTH,7-today+type);
        }
        //Date date1 = new Date(year, month, date, hrs, min);
        //c.setTime(date1);
